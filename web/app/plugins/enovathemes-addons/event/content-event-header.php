<?php
    ...
    $event_title_parallax               = (isset($GLOBALS['goodresto_enovathemes']['event-title-parallax']) && $GLOBALS['goodresto_enovathemes']['event-title-parallax'] == 1) ? "true" : "false";
    $event_title_opacity                = (isset($GLOBALS['goodresto_enovathemes']['page-title-opacity']) && $GLOBALS['goodresto_enovathemes']['page-title-opacity'] == 1) ? "true" : "false";
    $event_title_text                   = (isset($GLOBALS['goodresto_enovathemes']['event-title-text']) && $GLOBALS['goodresto_enovathemes']['event-title-text']) ? $GLOBALS['goodresto_enovathemes']['event-title-text'] : '';
    $event_subtitle_text                = (isset($GLOBALS['goodresto_enovathemes']['event-subtitle-text']) && $GLOBALS['goodresto_enovathemes']['event-subtitle-text']) ? $GLOBALS['goodresto_enovathemes']['event-subtitle-text'] : '';
    
    $event_breadcrumbs                  = (isset($GLOBALS['goodresto_enovathemes']['event-breadcrumbs']) && $GLOBALS['goodresto_enovathemes']['event-breadcrumbs'] == 1) ? "true" : "false";
    ...
    $slider                    = isset($slider_settings["event_slider_id"]) ? $slider_settings["event_slider_id"] : "none";
...
?>
<?php if(shortcode_exists("rev_slider") && $slider != "none" && is_post_type_archive('event')): ?>
    ...
    <?php endif ?>
<?php else: ?>
    <?php if ($event_title != "false"): ?>
        <header class="rich-header event-header <?php echo $page_title_text_align; ?> <?php echo $page_title_text_align_mob; ?>" data-opacity="<?php echo $event_title_opacity; ?>" data-parallax="<?php echo esc_attr($event_title_parallax); ?>" data-fixed="<?php echo esc_attr($event_title_parallax); ?>">
            <?php if ($event_title_parallax == "true"): ?>
                <div class="parallax-container">&nbsp;</div>
            <?php endif ?>
            <?php if ($event_title_back_img_attachment == "fixed"): ?>
                <div class="fixed-container">&nbsp;</div>
            <?php endif ?>
            <div class="container">
                <div class="rh-content et-clearfix">
                    <div class="rh-title">
                        <?php if (is_tax('event-category') || is_tax('event-tag')): ?>
                            <h1><?php echo single_cat_title("", false); ?></h1>
                        <?php else: ?>
                            <?php if (!empty($event_title_text)): ?>
                                <h1><?php echo esc_attr($event_title_text); ?></h1>
                            <?php else: ?>
                                <h1><?php echo esc_html__("Events", 'enovathemes-addons'); ?></h1>
                            <?php endif ?>
                        <?php endif; ?>  
                        <?php if (!empty($event_subtitle_text)): ?>
                            <div class="separator"></div>
                            <p><?php echo esc_attr($event_subtitle_text); ?></p>
                        <?php endif ?>                      
                    </div>
                    <?php if ($event_breadcrumbs == "true" && function_exists('enovathemes_addons_breadcrumbs')): ?>
